...
static esp_err_t gpio13Off_handler(httpd_req_t *req){
    /*Serial.println("OFF, 13.port LOW");
    digitalWrite(13, LOW);
    */
...
static esp_err_t deneme_handler(httpd_req_t *req){
  /*
    if(start == true){

      Serial.println("12 HIGHABC");
      start = false;
    }

    if(millis() >= time_0 + 1000){

        Serial.println("12 LOWWWWWW");
        time_0 +=9999;
...
void startCameraServer(){
   ...
   httpd_uri_t stream_uri = {
    ...
    httpd_uri_t deneme_uri = {
        .uri = "/deneme",
        .method = HTTP_GET,
        .handler = deneme_handler,
        .user_ctx = NULL
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &gpio12On_uri);
        httpd_register_uri_handler(camera_httpd, &gpio12Off_uri);
        httpd_register_uri_handler(camera_httpd, &gpio13On_uri);
        httpd_register_uri_handler(camera_httpd, &deneme_uri);
        httpd_register_uri_handler(camera_httpd, &gpio13Off_uri);
